/* ═══════════════════════════════════════════════════════════════════════════
   ACAISTACK INDEX PAGE - Homepage-only managers (chat, hero slider, portfolio)
   Split from app.js for performance - only loaded on index.html
   ═════════════════════════════════════════════════════════════════════════ */
...
/**
 * INITIALIZATION - Index-only managers (loaded only on homepage)
 */
function initIndexManagers() {
  const managers = [
  ...
  const init = () => {
    managers.forEach(({ name, instance }) => {
      try {
        instance.init();
        Logger.info(`${name} initialized (index-only)`);
      } catch (error) {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', initIndexManagers);
} else {
  initIndexManagers();
}
